...
driver <- rsDriver(browser = c("chrome"), chromever = "93.0.4577.15")
remote_driver <- driver[["client"]]

remote_driver$open()

#Navigate the web page
remote_driver$navigate("https://app.cnecuentasclaras.gov.co/CuentasClarasPublicoTer2019/Consultas/Candidatos/")
...
mpio_fun <- function(mpio, dpto){
  ...
  mpio_data <- data.frame(corp=cand_corp,
                          ...
                          party=cand_party,
                          wp_no=cand_number)
  mpiopath <- paste0("data/auxiliar/names/2019/",
                     corp_name, "_",
                     dpto_name, "_", 
                     mpio_name,".csv")
  #Export in a csv
 ...
 while(x<repetitions){
   ...
   x <- x+1
     ...
     dpto_data <- data.frame(corp=cand_corp,
                             ...
                             party=cand_party,
                             wp_no=cand_number)
     dptopath <- paste0("data/auxiliar/names/2019/",corp_name, 
                        "_", dpto_name,".csv")
     #Export in a csv
...
write_csv(candidates,
          path = 'data/auxiliar/allcandidatesnames19.csv')
...
download_donors_fun <- function(cand_number){
  ...
  basic_url <- 'https://app.cnecuentasclaras.gov.co/CuentasClarasPublicoTer2019/Consultas/Candidato/Formulario52xls/'
  ...
  donors_sheet <- donors_sheet%>%
    select(name=`Nombre de la Persona Natural o Jurídica`,
           ...
           don_inkind=`Donación en especie`)%>%
...
candidates_no_vtr <- candidates$wp_no%>%
  as_vector(.)
#Discard candidates to municipality council
corp_list <- c("Asamblea", "Gobernación","Alcaldía")
candidates_no_vtr_wocityc <- candidates%>%
  filter(corp%in%corp_list)%>%
  .$wp_no%>%as_vector(.)

candidates_no_vtr_cityc <- candidates%>%
  filter(corp=="Concejo")%>%
  .$wp_no%>%as_vector(.)

candidates_no_vtr_cityc_1 <- candidates_no_vtr_cityc[1:20000]

donors_raw_wocityc <- lapply(candidates_no_vtr_wocityc,
                     download_donors_fun)

donors_raw_cityc1 <- lapply(candidates_no_vtr_cityc_1, 
                           download_donors_fun)

donors_raw_wocityc_df <- donors_raw_wocityc%>%
  bind_rows(.)%>%
  filter(!is.na(cand_number))

write_csv(donors_raw_wocityc_df,"data/auxiliar/names/donors_2019.csv")